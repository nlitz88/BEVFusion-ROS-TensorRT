...
void BEVFusionNode::free_images(std::vector<unsigned char*>& images) 
{
...
std::shared_ptr<bevfusion::Core> BEVFusionNode::create_core(const std::string& model, const std::string& precision) 
{
  ...
  scn.model = pkg_path_ + nv::format("/model/%s/lidar.backbone.xyz.onnx", model.c_str());
  ...
  transbbox.model = pkg_path_ + nv::format("/model/%s/build/head.bbox.plan", model.c_str());
  transbbox.confidence_threshold = 0.12f;
  transbbox.sorted_bboxes = true;

  bevfusion::CoreParameter param;
  param.camera_model = pkg_path_ + nv::format("/model/%s/build/camera.backbone.plan", model.c_str());
  param.normalize = normalization;
  param.lidar_scn = scn;
  param.geometry = geometry;
  param.transfusion = pkg_path_ + nv::format("/model/%s/build/fuser.plan", model.c_str());
  param.transbbox = transbbox;
  param.camera_vtransform = pkg_path_ + nv::format("/model/%s/build/camera.vtransform.plan", model.c_str());
  return bevfusion::create_core(param);
}


BEVFusionNode::BEVFusionNode(const std::string& model_name, const std::string& precision, const std::string& pkg_path)
  : model_name_(model_name), precision_(precision), pkg_path_(pkg_path)
{ 
  
  // std::cout << "\033[1;32m--model_name: " << model_name_ << "\033[0m" << std::endl;
  // std::cout << "\033[1;32m--precision : " << precision_ << "\033[0m" << std::endl;
  // std::cout << "\033[1;32m--pkg_path : " << pkg_path_ << "\033[0m" << std::endl;


  config_path_ = pkg_path_ + "/configs";
  
  // std::cout << "\033[1;32m--config_path : " << config_path_ << "\033[0m" << std::endl;
  ...
  // tensor读取配置参数
  camera2lidar = nv::Tensor::load(config_path_ + "/camera2lidar.tensor", false);
  camera_intrinsics = nv::Tensor::load(config_path_ + "/camera_intrinsics.tensor", false);
  lidar2image = nv::Tensor::load(config_path_ + "/lidar2image.tensor", false);
  img_aug_matrix = nv::Tensor::load(config_path_ + "/img_aug_matrix.tensor", false);
